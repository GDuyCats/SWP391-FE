...
function RequestManagement() {
        ...
        try {
            ...
            if (res.status === 200) {
                ...
                const pendingRequests = allRequests.filter(req =>
                    req.status !== "accept" && req.status !== "accepted"
                );
    ...
    async function acceptRequest(id) {
        try {
            ...
            if (res.status === 200) {
                ...
                // TỰ ĐỘNG ẨN YÊU CẦU SAU KHI CHẤP NHẬN
    ...
    const getStatusBadge = (status) => {
        if (status === "pending") return <span className="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">Đang chờ</span>;
        if (status === "accepted") return <span className="px-2 py-0.5 bg-green-100 text-green-700 rounded-full text-xs font-medium">Đã chấp nhận</span>;
        return null;
    };
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6">
                            ...
                            <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5">
                                ...
                                <div className="flex gap-2 lg:flex-col lg:items-end">
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all font-medium shadow-md hover:shadow-lg transform hover:-translate-y-0.5 text-sm whitespace-nowrap"
                                    >
            ...
            {toast && <Toast type={type} msg={msg} onClose={() => setToast(false)} />}
        </div>
    );
}
