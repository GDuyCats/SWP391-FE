...
export default function TransactionDetail() {
    ...
    const SEND_DRAFT = "/staff/contracts/send-draft"; // ĐÃ CÓ LẠI
    ...
    const handleEditPrice = () => {
        setCarPriceInput(String(getCarPrice()));
    ...
    const buildEmailFor = (aud) => {
        ...
        const feeLines = feeTypes.map(key => {
            const payer = feePayer[key];
            const amount = payer === "buyer" ? toNumber(fees[key].buyer) : payer === "seller" ? toNumber(fees[key].seller) : 0;
            if (amount <= 0) return null;
            return `<b>${feeLabels[key]}:</b> ${formatCurrency(amount)} VNĐ <span style="color:#666">(${payer === "buyer" ? "Buyer" : "Seller"} chịu)</span>`;
        }).filter(Boolean);

        if (aud === "buyer") {
            const ctaUrl = buildBuyerContractUrl();
            const subject = `[EV] Hợp đồng #${recordData.id} – Bên A | Phí ${formatCurrency(buyerFee)}đ | Tổng ${formatCurrency(buyerTotal)}đ`;
            const lines = [
            ...
            return {
                ...
                html: makeEmailHtml({ title: `Hồ sơ #${recordData.id} – Bên A`, lines, ctaUrl, ctaLabel: "Mở Contract A & Nhập OTP" }),
                ctaUrl,
            };
        }

        const ctaUrl = buildSellerContractUrl();
        const subject = `[EV] Hợp đồng #${recordData.id} – Bên B | Phí ${formatCurrency(sellerFee)}đ | Nhận ${formatCurrency(sellerTotal)}đ`;
        const lines = [
            `Giá chốt: <b>${formatCurrency(price)} VNĐ</b>`,
        ...
        return {
            ...
            subject,
            text: lines.map(l => l.replace(/<[^>]*>/g, '')).join("\n") + `\nMở: ${ctaUrl}`,
            html: makeEmailHtml({ title: `Hồ sơ #${recordData.id} – Bên B`, lines, ctaUrl, ctaLabel: "Mở Contract B & Nhập OTP" }),
            ctaUrl,
        };
    };

    // GỬI CHI TIẾT PHÍ + AI CHỊU
    ...
    return {
        ...
        feeDetails, // ✅ luôn có payer + amount
        toRole: email.toRole,
    ...
    const sendDraftOne = async (aud) => {
        ...
        try {
            ...
            console.error("sendDraftOne error", e);
            throw e;
        }
    };
    ...
    const handleSendRecordToBoth = async () => {
        ...
        try {
            ...
            setToast({ type: "success", message: "Đã gửi hồ sơ cho cả hai bên" });
        } catch (error) {
            setToast({ type: "error", message: error?.response?.data?.message || "Gửi thất bại" });
        } finally {
    ...
    const handleSendOtpCode = async () => {
        ...
        try {
            ...
            setToast({ type: "error", message: error?.response?.data?.message || "Gửi OTP thất bại" });
        } finally {
            setOtpSending(false);
        }
    };
    ...
    const handleFinalize = async () => {
        if (!recordData?.id || finalCarPrice === null || !validateFees()) return;
        setFinalSending(true);
        try {
            const payload = {
                contractId: recordData.id,
                agreedPrice: getCarPrice(),
                buyerFee: getBuyerFee(),
                sellerFee: getSellerFee(),
            };
            ...
            <div className="flex">
                ...
                <main className="flex-1 py-8 px-6">
                    <div className="max-w-7xl mx-auto">
                        {/* Header */}
                        <div className="mb-6">
                            <button onClick={() => navigate("/transactionrecords")} className="flex items-center text-gray-600 hover:text-gray-900 mb-4">
                        ...
                        <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg shadow-sm border-2 border-yellow-300 p-6 mb-6">
                            ...
                            {!isEditingPrice ? (
                                    ...
                                    <div>
                                        ...
                                        {finalCarPrice !== null && (
                                            <p className="text-xs text-green-600 mt-1 flex items-center">
                                                <CheckCircle className="w-4 h-4 mr-1" />Đã chốt
                                            </p>
                                        )}
                                    </div>
                                    <button
                                ...
                                <div className="flex gap-3 items-start">
                                    ...
                                    <button
                                        onClick={() => { setCarPriceInput(""); setIsEditingPrice(false); }}
                                ...
                                <div className="p-6 space-y-4">
                                    {feeTypes.map((key) => (
                                        <div key={key} className="flex items-center gap-3">
                                            ...
                                            />
                                            <label className="flex-1 text-sm font-medium text-gray-700">{feeLabels[key]}</label>
                                            <input
                                                type="text"
                                                inputMode="numeric"
                                    ...
                                    <div className="mt-6 p-4 bg-blue-50 rounded-lg">
                                        <p className="text-sm text-gray-600 mb-2">Giá trị xe: <span className="font-semibold">{formatCurrency(getCarPrice())} VNĐ</span></p>
                                        <p className="text-sm text-gray-600">Tổng phí Buyer: <span className="font-semibold text-blue-600 text-lg">{hideBuyerDetails ? maskText : `${formatCurrency(getBuyerFee())} VNĐ`}</span></p>
                                    </div>
                                    <div className="pt-4 border-t border-gray-200">
                                        <div className="flex justify-between items-center">
                                            <span className="text-lg font-semibold">Tổng thanh toán:</span>
                                            <span className="text-xl font-bold text-blue-600">{hideBuyerDetails ? maskText : `${formatCurrency(getCarPrice() + getBuyerFee())} VNĐ`}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-white rounded-lg shadow-sm border border-green-200 overflow-hidden">
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    <div className="mt-6 p-4 bg-green-50 rounded-lg">
                                        <p className="text-sm text-gray-600 mb-2">Giá trị xe: <span className="font-semibold">{formatCurrency(getCarPrice())} VNĐ</span></p>
                                        <p className="text-sm text-gray-600">Tổng phí Seller: <span className="font-semibold text-green-600 text-lg">{hideSellerDetails ? maskText : `${formatCurrency(getSellerFee())} VNĐ`}</span></p>
                                    </div>
                                    <div className="pt-4 border-t border-gray-200">
                                        <div className="flex justify-between items-center">
                                            <span className="text-lg font-semibold">Tổng nhận được:</span>
                                            <span className="text-xl font-bold text-green-600">{hideSellerDetails ? maskText : `${formatCurrency(getCarPrice() - getSellerFee())} VNĐ`}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
                            ...
                            {(buyerRecordSent || sellerRecordSent) && (
                                <div className="mb-4 p-3 bg-green-50 border border-green-200 rounded-lg text-sm text-green-700 font-medium flex items-center gap-2">
                                    ...
                                    {buyerRecordSent && sellerRecordSent ? "Đã gửi cả hai bên" : buyerRecordSent ? "Đã gửi cho Buyer" : "Đã gửi cho Seller"}
                                </div>
                            )}
                            <button
                            ...
                            >
                                ...
                                {buyerRecordSent && sellerRecordSent ? "Đã gửi" : buyerSending || sellerSending ? "Đang gửi..." : "Gửi cho cả hai bên"}
                            </button>
                        </div>
                                ...
                                <div className="flex gap-3">
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-6 py-3 rounded-lg border font-medium transition-colors ${
                                            otpSent ? "border-gray-300 text-gray-400" :
                                            otpSending ? "border-blue-300 text-blue-300 cursor-wait" :
                                    ...
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-all shadow-lg ${
                                            finalSending ? "bg-gray-400 text-white cursor-wait" : "bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:from-purple-700 hover:to-pink-700"
                                        }`}
                                    >
                                        <CheckCircle className="w-5 h-5" />
